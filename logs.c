...
#define TRIP_LOG_COUNT 8
#define TRIP_LOG_MAGIC_0 0xA5
#define TRIP_LOG_MAGIC_1 0x5A
#define TRIP_LOG_VERSION 1
#define TRIP_LOG_HEADER_SIZE 6
#define TRIP_LOG_RECORD_SIZE 3
#define TRIP_LOG_STORAGE_SIZE \
  (TRIP_LOG_HEADER_SIZE + (TRIP_LOG_COUNT * TRIP_LOG_RECORD_SIZE))
#define EEPROM_LOG_START_ADDR \
  ((((uint16_t)sizeof(parameters) + EEPROM_PAGE_SIZE - 1U) / EEPROM_PAGE_SIZE) * \
   EEPROM_PAGE_SIZE)
...
uint8_t logs[TRIP_LOG_COUNT] = {0,0,0,0,0,0,0,0};
uint16_t log_values[TRIP_LOG_COUNT] = {0,0,0,0,0,0,0,0};
...
static uint16_t trip_log_checksum(const uint8_t *buffer, uint16_t length) {
  uint16_t checksum = 0;

  for (uint16_t index = 0; index < length; ++index) {
    checksum = (uint16_t)(checksum + buffer[index]);
  }

  return checksum;
}

static void clear_trip_logs(void) {
  memset(logs, 0, sizeof(logs));
  memset(log_values, 0, sizeof(log_values));
  total_logs = 0;
}

static int save_trip_logs(uint16_t startAddr) {
  uint8_t buffer[TRIP_LOG_STORAGE_SIZE];
  uint16_t offset = 0;
  uint16_t status;

  memset(buffer, 0, sizeof(buffer));
  buffer[0] = TRIP_LOG_MAGIC_0;
  buffer[1] = TRIP_LOG_MAGIC_1;
  buffer[2] = TRIP_LOG_VERSION;
  buffer[3] = total_logs;

  for (uint16_t index = 0; index < TRIP_LOG_COUNT; ++index) {
    uint16_t base = TRIP_LOG_HEADER_SIZE + (index * TRIP_LOG_RECORD_SIZE);
    buffer[base] = logs[index];
    buffer[base + 1] = (uint8_t)(log_values[index] & 0xFFU);
    buffer[base + 2] = (uint8_t)((log_values[index] >> 8) & 0xFFU);
  }

  uint16_t checksum = trip_log_checksum(buffer, TRIP_LOG_STORAGE_SIZE);
  buffer[4] = (uint8_t)(checksum & 0xFFU);
  buffer[5] = (uint8_t)((checksum >> 8) & 0xFFU);

  while (offset < TRIP_LOG_STORAGE_SIZE) {
    uint16_t chunk =
        (TRIP_LOG_STORAGE_SIZE - offset > EEPROM_PAGE_SIZE) ? EEPROM_PAGE_SIZE : TRIP_LOG_STORAGE_SIZE - offset;

    ControlAddr = startAddr + offset;
    EEPROM.pTX_MsgBuffer = &buffer[offset];
...
static int load_trip_logs(uint16_t startAddr) {
  uint8_t buffer[TRIP_LOG_STORAGE_SIZE];
  uint16_t offset = 0;
  uint16_t status;
  ...
  while (offset < TRIP_LOG_STORAGE_SIZE) {
    ...
    EEPROM.NumOfDataBytes = chunk;
    EEPROM.pRX_MsgBuffer = &buffer[offset];
    status = I2C_ControllerReceiver(&EEPROM);

    offset += chunk;
  }

  if (buffer[0] != TRIP_LOG_MAGIC_0 || buffer[1] != TRIP_LOG_MAGIC_1 ||
      buffer[2] != TRIP_LOG_VERSION) {
    clear_trip_logs();
    save_trip_logs(EEPROM_LOG_START_ADDR);
    return 1;
  }

  uint16_t stored_checksum = (uint16_t)(buffer[4] | ((uint16_t)buffer[5] << 8));
  buffer[4] = 0;
  buffer[5] = 0;

  if (stored_checksum != trip_log_checksum(buffer, TRIP_LOG_STORAGE_SIZE) ||
      buffer[3] > TRIP_LOG_COUNT) {
    clear_trip_logs();
    save_trip_logs(EEPROM_LOG_START_ADDR);
    return 1;
  }
  ...
  for (uint16_t index = 0; index < TRIP_LOG_COUNT; ++index) {
    uint16_t base = TRIP_LOG_HEADER_SIZE + (index * TRIP_LOG_RECORD_SIZE);
    logs[index] = buffer[base];
    log_values[index] = (uint16_t)buffer[base + 1] |
                        ((uint16_t)buffer[base + 2] << 8);
  }

  return 0;
}

static void append_trip_log(uint8_t log_type, uint16_t log_value) {
  uint8_t slot;

  if (total_logs < TRIP_LOG_COUNT) {
    slot = total_logs;
    ++total_logs;
  } else {
    memmove(&logs[0], &logs[1], TRIP_LOG_COUNT - 1);
    memmove(&log_values[0], &log_values[1], (TRIP_LOG_COUNT - 1) * sizeof(uint16_t));
    slot = TRIP_LOG_COUNT - 1;
  }

  logs[slot] = log_type;
  log_values[slot] = log_value;
  save_trip_logs(EEPROM_LOG_START_ADDR);
}
...
void initADCA0(void) {
  ...
  ADC_setupSOC(ADCA_BASE, ADC_SOC_NUMBER1, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN11,
               80);
  ADC_setupSOC(ADCA_BASE, ADC_SOC_NUMBER2, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN7,
               15);
...
void main(void) {
  ...
  load_trip_logs(EEPROM_LOG_START_ADDR);
...
void UI_Task(void *pvParameters) {
  ...
  int earth_vol, t_start = 5, f, parameter_pre_temp = 4, hlt_timer = 0,
                 contactor_vol = 0, motor_c = 0,log_couter=0;
  ...
  for (;;) {
    ...
    case TRIP_LOGS:
      ...
      lcd_setCursor(0,1);
        ...
        if (log_couter >= total_logs) {
          log_couter = total_logs - 1;
        }
       ...
       break;
       case 1:
      lcd_printf("IP HIGH - %4d",log_values[log_couter]);
       break;
       case 2:
      lcd_printf("IP LOW - %4d",log_values[log_couter]);
       break;
       case 3:
      lcd_printf("OP HIGH - %4d",log_values[log_couter]);
       break;
       case 4:
      lcd_printf("OP LOW - %4d",log_values[log_couter]);
       break;
       case 5:
      lcd_printf("OL-1 - %4d",log_values[log_couter]);
       break;
        case 6:
      lcd_printf("OL-2 - %4d",log_values[log_couter]);
       break;
        case 7:
      ...
      if (GPIO_readPin(UP) == 1 && up_flag == 0 && log_couter + 1 < total_logs) {
       ++log_couter;
      }
    ...
    if (v_in >
      ...
      if (hlt_timer > (store_data.hi_lo_time_s * 5)) {
        relay_flag = 0;
        contactor_voltage_count=0;
        append_trip_log(1, (uint16_t)v_in);
        buzer_off();
        v_in_high = 1;
      } else {
        buzer_state = 1;
      ...
      if (hlt_timer > (store_data.hi_lo_time_s * 5)) {
        ...
        relay_flag = 0;
        contactor_voltage_count=0;
        append_trip_log(2, (uint16_t)v_in);
      ...
      if (hlt_timer > (store_data.hi_lo_time_s * 5)) {
        relay_flag = 0;
        contactor_voltage_count=0;
        append_trip_log(3, (uint16_t)v_o);
        buzer_off();
        op_high = 1;
      } else {
        buzer_state = 1;
      ...
      if (hlt_timer > (store_data.hi_lo_time_s * 5)) {
        relay_flag = 0;
        contactor_voltage_count=0;
        append_trip_log(4, (uint16_t)v_o);
        if(start_flag == 1) {
        buzer_off();
        }
        op_low = 1;
