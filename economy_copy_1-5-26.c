...
#define MIN_OL1_TIME 30 // default time is 90 sec
...
unsigned char display_page_index = 0, display_page_start = 0, display_page_end = 2;
...
void start_manual_restart_delay_for_cur(void);
//--------------------------------------------------------------------------------
        ...
        case 3:
            ...
            if (trip_elapsed_ticks > overload_trip_ticks)
            {
                ...
                else
                {
                    ...
                    start_manual_restart_delay_for_cur();
                    output_current_rms = 0;
                    P00 = 0;
                    relay_enabled = 0;
                    display_page_index = 1;
...
void timer0_isr(void) interrupt 1
{
    ...
    {
        if (startup_delay_profile != 0 && BTN_DOWN == 1)// manual reset btn
        {
            start_manual_restart_delay();
            return;
        }
        ...
        buzzer_on();
        if (startup_delay_profile != 0 && ((startup_timer_ticks / ticks_per_second) & 1) == 0)
        {
...
void start_manual_restart_delay_for_cur(void)
{
    power_on_delay_ticks = (unsigned int)(1 * 2);
    ticks_per_second = 2;
    startup_countdown_seconds = (unsigned char)(power_on_delay_ticks / 2);
    startup_delay_profile = 0;
    startup_complete = 0;
    startup_delay_active = 1;
    startup_timer_ticks = 0;
...
void start_overload_auto_recovery(unsigned char profile)
{
    power_on_delay_ticks = 120U * 12U;
    ticks_per_second = 12;
    overload_recovery_seconds = 120;
...
void main(void)
{
    ...
    if (load_config_from_flash() == 0)
    {
        ...
        g_config.overload_trip_ticks = 90;
    ...
    while (1)
    {
        ...
        else if (startup_complete == 1)
        {
            ...
            if (input_high_latched == 1)
            {
                ...
                if (input_voltage < (input_high_limit - 10))
                {
            ...
            {
                if (input_voltage > (input_low_limit + 10))
                {
                    start_reconnect_delay();
                    set_status_led(LED_HILO, 0);
                    input_low_latched = 0;
        ...
        if (display_refresh_pending && (startup_delay_active == 0 || startup_delay_profile == 0))
        {
            ...
            else
            {
                ...
                case CON_OFF:
                    ...
                    if (is_set_key_pressed())
                    {
											if(g_config.earth_monitor_enabled == 1) {
											g_ui_state = EARTH_ON;
											}
											else {
