...
typedef struct {
  ...
  uint8_t ondelay_s;
  uint32_t ip_low_voltage;
  uint32_t ip_high_voltage;
   uint32_t op_high_voltage;
  uint32_t op_low_voltage;
  uint8_t hi_lo_time_s;
  uint16_t input_calibration;
  uint16_t output_calibration;
...
void load_default_parameters(parameters *p)
{
    ...
    p->ip_high_voltage = 280;
    p->op_high_voltage = 290;
    p->op_low_voltage = 180;
    p->hi_lo_time_s = 5;
    p->input_calibration = 1.0f;
    p->output_calibration = 1.0f;
    p->current_calibration = 1.0f;
...
void Control_Task(void *pvParameters) {
  uint8_t output_regulation = 3,flag=0;
...
switch(pre_state) {
 ...
 break;
  ...
  case IP_CALIBRATION:
  store_data.input_calibration = (float)new_interger_data/v_in;
  break;

   case OP_CALIBRATION:
   store_data.output_calibration = (float)new_interger_data/v_o;
   break;

   case OA_CALIBRATION:
  store_data.current_calibration = (float)new_interger_data/c;
  break;
...
//--gama------
  int earth_vol,t_start=5,f,parameter_pre_temp = 4,hlt_timer=0,contactor_vol=0;
  float motor_c;
  ...
  for (;;) {
    ...
    required_parameter.set_volt = store_data.target_output_voltage;
    xSemaphoreGive(parameter_mutex);
   ...
   if(GPIO_readPin(227) == 0) {
    ...
    case IP_LOW:
      lcd_printf("IP LOW: ");
      lcd_printf("%4d",(int) store_data.ip_low_voltage);
      lcd_setCursor(0, 1);
      lcd_printf("<-- EDIT:P07 -->");
      if (GPIO_readPin(UP) == 0) {
      ...
      if (GPIO_readPin(SET) == 0) {
        edit_screen = 1;
        pre_state = display_state;
        set_number((int)store_data.ip_low_voltage);
        display_state = EDIT;
      }
      break;
    ...
    case IP_HIGH:
      lcd_printf("IP HIGH: ");
      lcd_printf("%4d", (int)store_data.ip_high_voltage);
      lcd_setCursor(0, 1);
      lcd_printf("<-- EDIT:P07 -->");
      if (GPIO_readPin(UP) == 0) {
      ...
      if (GPIO_readPin(SET) == 0) {
        edit_screen = 1;
        pre_state = display_state;
        set_number((int)store_data.ip_high_voltage);
        display_state = EDIT;
      }
      break;
    ...
    case OP_LOW:
      lcd_printf("OP LOW: ");
      lcd_printf("%4d", (int)store_data.op_low_voltage);
      lcd_setCursor(0, 1);
      lcd_printf("<-- EDIT:P07 -->");
      if (GPIO_readPin(UP) == 0) {
      ...
      if (GPIO_readPin(SET) == 0) {
        edit_screen = 1;
        pre_state = display_state;
        set_number((int)store_data.op_low_voltage);
        display_state = EDIT;
      }
      break;
    ...
    case OP_HIGH:
      lcd_printf("OP HIGH: ");
      lcd_printf("%4d", (int)store_data.op_high_voltage);
      lcd_setCursor(0, 1);
      lcd_printf("<-- EDIT:P07 -->");
      if (GPIO_readPin(UP) == 0) {
        display_state = HI_LO_TIME;
      ...
      if (GPIO_readPin(SET) == 0) {
        edit_screen = 1;
        pre_state = display_state;
        set_number((int)store_data.op_high_voltage);
        display_state = EDIT;
      }
      break;
...
if(v_in > store_data.ip_high_voltage && v_in_high == 0) {
  display_error = 1;
++hlt_timer;
...
else if(v_in < store_data.ip_low_voltage&& v_in_low == 0) {
...
else if(v_o > store_data.op_high_voltage&& op_high == 0){
  display_error = 3;
++hlt_timer;
if(hlt_timer > (store_data.hi_lo_time_s * 5)) {
relay_flag = 0;
...
else if(v_o < store_data.op_low_voltage && op_low == 0) {
  display_error = 4;
++hlt_timer;
if(hlt_timer > (store_data.hi_lo_time_s * 5)) {
relay_flag = 0;
...
else if(relay_flag == 1 && contactor_vol < 120 && v_in > 150) {
  display_error = 6;
++hlt_timer;
if(hlt_timer > (5 * 5)) {
relay_flag = 0;
...
if(v_in_high && v_in < (store_data.ip_high_voltage - 15)) {
relay_flag = 1;
v_in_high = 0;
}
if(v_in_low && v_in > (store_data.ip_low_voltage + 15)) {
relay_flag = 1;
v_in_low = 0;
}
if(op_high && v_o < (store_data.op_high_voltage - 15)) {
relay_flag = 1;
op_high = 0;
}
if(op_low && v_o > (store_data.op_low_voltage + 15)) {
relay_flag = 1;
op_low = 0;
}
